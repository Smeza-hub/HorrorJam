...
@export var flicker_intensity: float = 1   # Max variation in energy
@export var flicker_speed: float = 20.0      # How fast it flickers
@export var base_energy: float = 5         # The "normal" brightness
...
func spirit_light(delta):
	...
	if flicker_enabled and self is Light2D:
		...
		$PointLight2D.energy = base_energy + flicker
func fade_in():
	color.a = move_toward(color.a,1,10)
